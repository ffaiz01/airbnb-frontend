  ...
  try {
    ...
    const fetchPrices = async () => {
      try {
          ...
          for (let i = 0; i < pricingData.oneNight.length; i++) {
            ...
            if (item.checkin && item.checkout) {
              ...
              if (price !== null) {
                ...
                await Search.findByIdAndUpdate(params.id, { pricingData });
        ...
        await Search.findByIdAndUpdate(params.id, { status: 'completed' });
      } catch (error) {
        console.error('Error in background price fetching:', error);
        await Search.findByIdAndUpdate(params.id, { status: 'error' });
      }
    };
